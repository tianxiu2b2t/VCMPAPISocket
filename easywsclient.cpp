...
namespace { // private module-only namespace
    ...
    easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
        ...
        }
        {
            ...
            std::vector<std::string> requests = std::vector<std::string>{
                "GET /" + std::string(path) + " HTTP/1.1",
                "Host: " + std::string(host) + (port != 80 ? ":" + std::to_string(port) : ""),
                "Upgrade: websocket",
                "Connection: Upgrade",
            ...
            request += "\r\n";
            const char* request_c = request.c_str();
            ::send(sockfd, request_c, strlen(request_c), 0);
            char line[16384];
